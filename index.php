...
<body class="bg">
    ...
    if (!isset($_SESSION["etapa"]) || $_SESSION["etapa"] == "1") {
        ...
        echo montarLinkImagem('./index.php?etapa=2&layout=mobile', 'Mobile', 'col-md-4', 'mobile.png', '200');
        echo montarLinkImagem('./index.php?etapa=15', 'Manual', 'col-md-4', 'livro.png', '200', 'target="_self"');
        echo montarLinkImagem('./index.php?etapa=2&layout=desktop', 'Desktop', 'col-md-4', 'computador-borda.png', '200');
        echo '</div>';
        ...
        // Durante a localização se o escolhido for uma Humano disponibiza para selecionar uma das cartas caso assim tenha redirecionando para a expor na etapa (12)
        ...
        // Essa etapa é responsavel pela exibição dos palpites realizados pelo jogador humano na etapa anteiror (9)
		...
		// controlador utilizado apenas para atualizar o proximo destino que é feito atraves da etapa 14.
    ...
    if ($_SESSION["etapa"] > 1){        
        ...
        echo montarLinkReset();        
        if($_SESSION["etapa"] >= 8 && $_SESSION["etapa"] <= 14){
            echo montarModalFichaPalpite($_SESSION["jogadores"]);
            
        }
        
        if($_SESSION["etapa"] > 1 && $_SESSION["etapa"] < 15){
